    ...
    else
    {
        ...
        for(int i=0;i<non.size();i++)
        {
            for(int j=0;j<cir_junction_point[non[i]].size();j++)
            {
                ...
                int now=scc_place_map[cir_junction_point[non[i]][j]].second;
                auto it1=find(scc1[now].cir.begin(),scc1[now].cir.end(),non[0]);
                auto it2=find(scc1[now].cir.begin(),scc1[now].cir.end(),non[1]);
                if(it1==scc1[now].cir.end()||it2==scc1[now].cir.end())
                //同时位于其他环
        ...
        for(int i=0;i<cir_junction_points.size();i++)
        {
            ...
            for(int j=0;j<node1.input.size();j++)
            {
                ...
                {
                    cout<<scc1[scc_place_map[node1.input[j]].second].name<<" ";
...
int main(int argc,char *argv[])
{
    ...
    {   
        scc_osc.push_back(0);
        if(scc_vec[i].size()==1){scc_cir_count.push_back(0);continue;}
        cir_count=0;
        johnson(scc_vec[i][0].id,scc_vec[i]);
        scc_cir_count.push_back(cir_count);
    ...
    vector<vector<string>>result_4;
    for(int i=0;i<scc_vec.size();i++)
    {
        
        if(scc_vec[i].size()==1||scc_osc[i]!=1) continue;
        /*检查环路是否正确
        cout<<scc_cir_count[i]<<endl;
        for(int j=0;j<scc_vec[i].size();j++)
        {   
        ...
        */
        ...
        vector<int>vis_cir;
        vector<int>vis_cir_temp;
        vector<int>cir_num;
        vector<string>result;
        for(int j=0;j<scc_vec[i].size();j++){
            cir_num.push_back(scc_vec[i][j].cir.size());
        }
        for(int j=0;j<cir_count;j++){
            vis_cir.push_back(0);
            vis_cir_temp.push_back(0);
        }
        
        
        int left_cir=scc_cir_count[i];
        while(left_cir>0){
         int max_cir=-1;
         int delete_node=0;
        for(int j=0;j<cir_num.size();j++){
            if(max_cir<cir_num[j]){
                max_cir=cir_num[j];
                delete_node=j;
            }
        }
        left_cir-=max_cir;
        result.push_back(scc_vec[i][delete_node].e[0].name);
        for(int j=0;j<scc_vec[i][delete_node].cir.size();j++){
            if(vis_cir[scc_vec[i][delete_node].cir[j]]=0){
                vis_cir_temp[scc_vec[i][delete_node].cir[j]]=1;
            }
            vis_cir[scc_vec[i][delete_node].cir[j]]=1;
        }
        for(int k=0;k<scc_vec[i].size();k++){
            for(int h=0;h<scc_vec[i][k].cir.size();h++){
                if(vis_cir_temp[scc_vec[i][k].cir[h]]==1){
                    cir_num[k]--;
                }
            }
        }
        ...
        result_4.push_back(result);


    }
    ofstream outputFile4("result_4.txt");
    if(outputFile4.is_open()){
       for(int i=0;i<result_4.size();i++){
        sort(result_4[i].begin(),result_4[i].end());
        outputFile4<<i+1<<")"<<endl;
        outputFile4<<"Loop Breaker:";
        for(int j=0;j<result_4[i].size();j++){
            if(j==0)outputFile4<<result_4[i][j];
            else outputFile4<<","<<result_4[i][j];
